...
describe('docker packaging', () => {
  it('ships prebuilt frontend assets instead of running npm install inside the image build', () => {
    expect(dockerfileText).toContain('COPY dist/client /usr/share/nginx/html');
    expect(dockerfileText).not.toContain('RUN npm ci');
  });
